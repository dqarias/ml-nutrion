...
def estado_nutricional_department_analysis(df):
    ...
    # Filtrar solo los registros donde el estado nutricional es uno de los niveles de desnutrición
    ...
    X = df_desnutricion[department_columns]
    ...
    plt.title("Matriz de Confusión para Desnutrición")
    ...
    plt.title("Importancia de las Características (Departamentos) - Desnutrición")
    plt.xlabel("Importancia")
    plt.ylabel("Departamento")
    plt.show()

    # Identificar los departamentos más relacionados con los diagnósticos de desnutrición
    top_departments = feature_importances.head(5).index.tolist()
    print("\nDepartamentos más relacionados con el diagnóstico de Desnutrición:")
    for i, dept in enumerate(top_departments, 1):
        print(f"{i}. {dept}")

    # Distribución de desnutrición por departamentos
    desnutricion_distribution = df_desnutricion.groupby('Estado_nutricional')[department_columns].sum().T
    desnutricion_distribution.plot(kind='bar', stacked=True, figsize=(14, 8), colormap='coolwarm')
    plt.title("Distribución de Desnutrición por Departamento")
